  ...
  try {
    ...
    const [subscriptions]: any = await pool.execute(
      `SELECT 
        ...
        sp.plan_code AS plan_name,
        sp.price AS plan_price_per_month, -- Giá gói theo tháng
        spo.total_cost AS total_revenue, -- Tổng doanh thu từ subscriptionpendingorders
        TIMESTAMPDIFF(MONTH, os.start_date, os.end_date) AS months_subscribed, -- Số tháng đăng ký
